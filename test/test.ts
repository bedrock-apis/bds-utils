...
installation.config.setAllowed([...installation.config.getAllowed(), '@minecraft/server-net']);
if (!(await installation.worlds.get('level'))) {
   // Import it
   ...
   installation.worlds.setActive(world);
}

installation.events.add('dispose', async () => {
   const w = await installation.worlds.get('level');
   if (w) installation.worlds.delete(w);
});
